...
// @history        4.1.110104.2110 = Added the (currently hidden) option to move the referral stats.. Change the about:config preference 'statsSidebarPosition' to 'left' or 'right' to change its position; 
...
var availableGraphs = [
  ...
  logger('this.graphsOnCurrentPage');
  logger(this.graphsOnCurrentPage);
  logger('this.graphsOnCurrentPage.length = ' +this.graphsOnCurrentPage.length);
  ...
  for (var i = 0; i < this.graphsOnCurrentPage.length; i++)
  {
    ...
    logger('this.graphsOnCurrentPage[i]');
    logger(this.graphsOnCurrentPage[i]);
    logger('friendlyNameLookup[this.graphsOnCurrentPage[i]]');
    logger(friendlyNameLookup[this.graphsOnCurrentPage[i]]);

    var _currentGraph = _graphs[friendlyNameLookup[this.graphsOnCurrentPage[i]]];
...
infoLabel.innerHTML = tmp;
  ...
  var statsSidebarPosition = manipulatePrefs.getPref('statsSidebarPosition','right');
